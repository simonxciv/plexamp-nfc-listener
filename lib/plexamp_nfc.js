...
import process_plexamp_command, { pause_player } from './process_plexamp_command.js';
import fs from 'fs';

const { pauseOnCardRemoval } = JSON.parse(fs.readFileSync('settings.json', 'utf-8'));
		...
		reader.on('card', async (card) => {
			...
			try {
				...
				if (
					...
					await process_plexamp_command(uri);
				} else {
					console.log(
