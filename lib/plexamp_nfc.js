...
import { start_player, pause_player, resume_player } from './process_plexamp_command.js';
import fs from 'fs';
		...
		reader.on('card', async (card) => {
			...
			try {
				...
				if (
					...
					const newUri = new URL(NDEFMessage[0].uri);

					({ resumeOnCardPlacement } = JSON.parse(fs.readFileSync('settings.json', 'utf-8')));

					if (newUri?.href === uri?.href && resumeOnCardPlacement) {
						await resume_player();
					} else {
						uri = newUri;
