...
export const resume_player = async () => {
	console.log(`Card re-placed. Sending resume command to player ${playerIP}`);
	const res = await fetch(`${getPlayerUri()}/player/playback/play`);

	if (!res.ok) {
		throw new Error(`Unexpected error while resuming playback: ${res.status}`);
	}
